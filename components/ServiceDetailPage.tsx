...
const ServiceDetailPage: React.FC = () => {
  ...
  if (!service) {
    return (
      <div className="min-h-screen flex items-center justify-center pt-32">
        <div className="text-center">
          <h2 className="text-3xl font-black text-slate-400">
            Service Not Found
          </h2>
          <p className="text-slate-500 mt-3">
            The requested engineering practice does not exist.
          </p>
          <button
            onClick={() => navigate("/services")}
            className="mt-8 px-10 py-4 bg-[#2E1CFF] text-white rounded-xl font-black uppercase tracking-widest text-xs"
          >
            Back to Services
          </button>
        </div>
      ...
      {/* <section className="bg-slate-50 py-32 mb-32">
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {service.features?.map((feature, i) => (
            <div
              key={i}
              className="bg-white p-12 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all"
            >
              <h4 className="text-xl font-bold text-slate-950 mb-4">
                {feature}
              </h4>
              <p className="text-slate-500 text-sm font-medium">
                Engineered for scale, security, and elite performance.
              </p>
            </div>
          ))}
        </div>
